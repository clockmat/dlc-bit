...
class Hook:
    ...
    def on_sonicbit_download_not_found(self, sonicbit: SonicBit, download: Download) -> bool:
    ...
    def on_upload_error(self, sonicbit: SonicBit, download: Download, error: Exception) -> bool:
        """Called when an upload fails, return `True` for soft failure or `False` to mark the download as failed"""
        return False

    def on_upload_complete(self, sonicbit: SonicBit, download_dict: dict, files_uploaded: int):
